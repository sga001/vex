...
void
throughput_vex_bidder_cp::run_experiment(int iterations, int num_auctions, int num_bidders)
{
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    for (int j = 0; j < num_auctions; ++j) {
      c_req creq = gen_creq();
      creq.seed.resize(SEED_SIZE);
      uint8_t s[HASH_SIZE]; // not really needed...
      vex::setup(HASH_SIZE, g_prefix, creq.seed.data(), s);
